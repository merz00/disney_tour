...
void user_input_kari()
{
    string str = current_directory + "/input/test_input.csv";
    ifstream ifs(str);
    if(!ifs){
        cout << "入力エラー";
        return;
    ...
    while(getline(ifs, line)){
        ...
        if(loop == 0){
            for(int i = 0; i < strvec.size(); i++){
                most_atr.insert(stoi(strvec[i]));
            }
            loop++;
        }else if(loop == 1){
            for(int i = 0; i < strvec.size(); i++){
                med_atr.insert(stoi(strvec[i]));
            }
            loop++;
        }else{
            sttime = stoi(strvec[0]);
            endtime = stoi(strvec[1]);
            prepos = stoi(strvec[2]);
        }
    }
}
