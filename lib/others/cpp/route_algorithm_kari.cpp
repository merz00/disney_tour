...
//データの読み込み(ユーザーから)
{
	ifstream ifs("user_input.json", ios::binary);
    if(!ifs){
        cout << "入力エラー";
        return;
    }
	...
	endtime = string_to_step(o["start"].get<string>(), 1);
...
{
    //待ち時間の読み込み
	ifstream wait("wait_time_2017_4_3.csv");
    if(!wait){
        cout << "入力エラー";
        return;
    }
    ...
    //ファストパス時間の読み込み
    fp_time_read(7, "110_fastpass.csv");
    fp_time_read(9, "112_fastpass.csv");
    fp_time_read(17, "120_fastpass.csv");
    fp_time_read(20, "123_fastpass.csv");
    fp_time_read(27, "132_fastpass.csv");
    fp_time_read(28, "133_fastpass.csv");
    fp_time_read(29, "134_fastpass.csv");
    fp_time_read(32, "163_fastpass.csv");
    fp_time_read(36, "392_fastpass.csv");
...
//dfsが呼び出されるのは入場時,乗車後,ファストパス取得後のいずれかのタイミング
{
    ...
    if(!fp.empty()){
        ...
        for(int i = 0; i < nfp.size(); i++){
            if(fptime_end[nfp[i].first][nfp[i].second] < ptime + min_time + dtime[ride_cand[i].first][nfp[i].first]){
                fp_use = true;
...
//結果の出力
{
    ...
    for(int i = 0; i < cand_vec.size(); i++){
        picojson::object obj_at;
        obj_at.insert(make_pair("start", picojson::value(obj_st)));
        obj_at.insert(make_pair("attraction", cand_vec[i]));
        stringstream ss;
        ss << "cand" << (i+1);
        obj_all.insert(make_pair(ss.str(), picojson::value(obj_at)));
    }
	cout << picojson::value(obj_all).serialize(true) << endl; // trueだと整形あり
